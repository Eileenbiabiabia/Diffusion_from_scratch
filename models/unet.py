...
class SinusoidalEmbeddings(nn.Module):
    ...
    def forward(self, t):
        # to align with different GPU devices
...
class UNET(nn.Module):
    ...
    def forward(self, x, t):
        ...
        #print(f"After shallow_conv: {x.shape}")
        residuals = []
        #downsampling
        for i in range(self.num_layers//2):
            ...
            #print(f"↓ Layer {i+1} output: {x.shape}, residual: {r.shape}")
            residuals.append(r)
        #upsampling
        for i in range(self.num_layers//2, self.num_layers):
            layer = getattr(self, f'Layer{i+1}')
            up_x = layer(x, embeddings)[0]
            res_x = residuals[self.num_layers - i - 1]
            #print(f"↑ Layer {i+1} upsampled: {up_x.shape}, skip: {res_x.shape}")
            #x = torch.concat((layer(x, embeddings)[0], residuals[self.num_layers-i-1]), dim=1)
            if up_x.shape[2:] != res_x.shape[2:]:
                #print(f"⚠️ Mismatch at Layer {i+1}: up_x {up_x.shape}, res_x {res_x.shape}")
                res_x = F.interpolate(res_x, size=up_x.shape[2:], mode='nearest')
            x = torch.cat((up_x, res_x), dim=1)
